...
- **Normalização de path em `resolvePort`/`tokenFile`** (`servers/brain-server/lib/daemon-common.js`): strings de `dataDir` não-normalizadas (`-inline` vs `-<marketplace>`, separadores mistos) faziam processos derivarem porta/token DIFERENTES para a MESMA pasta lógica — o daemon escutava numa porta enquanto o cliente tentava outra, ou lia um token de um arquivo que o servidor nunca escreveu. Fix: normaliza o `dataDir` (path canônico) antes de derivar porta/token, em ambos os pontos.
- **`servers/brain-server/index.js` publicava o ponteiro ativo mas não seguia o próprio veredito** (`writeActivePointer(DATA_DIR)` seguido de uso incondicional de `DATA_DIR`): quando o anti-regression guard recusava a publicação (candidato mais leve/vazio que o ponteiro existente), o `index.js` continuava derivando porta/token/consolidação da pasta vazia — sem `brain-http.token` ali — enquanto todo hook sem env seguia o ponteiro para a OUTRA pasta, a real. Essa divergência ERA o 401. Fix: extraído `publishAndFollow(dir)` compartilhado em `scripts/lib/data-dir.js` — publica, relê o ponteiro, e retorna o valor do ponteiro se o candidato "perde" pra ele. Tanto `dataDir()` (branch de env) quanto `index.js` agora passam pela MESMA implementação, eliminando a possibilidade dos dois caminhos divergirem de novo.

Gate 699/0 local, `brain-health` confirmando probe ativo saudável (`{}`), CI verde na branch. PR #44.
