"""
rf_engine.dispatch — núcleo JSON-RPC 2.0 do MCP, AGNÓSTICO de transporte.

Contém as 9 tools do pipeline (contrato preservado byte-a-byte com o servidor
stdio original) e a função `handle(msg) -> dict | None`, que processa UMA
mensagem JSON-RPC e RETORNA a resposta (dict) ou None (para notificações).

Regra de ouro deste módulo: **sem I/O de transporte**. Não lê `sys.stdin`, não
escreve `sys.stdout`, não faz `print`. A casca de transporte (HTTP — o stdio
original foi removido na migração) chama `handle()` e serializa/entrega o
retorno. Diagnóstico de erro de tool vai por `logging` (stderr por padrão),
nunca pelo stdout — que corromperia o protocolo.

Migração STDIO -> Streamable HTTP: este núcleo foi extraído de `mcp_server.py`
sem alterar comportamento; o transporte é que trocou.
"""
...
logger = logging.getLogger("rf_engine.mcp")
...
# ─────────────────────────────────────────────────────────────────────────────
# Dispatch JSON-RPC 2.0 (retorna dicts; NÃO faz I/O de transporte)
# ─────────────────────────────────────────────────────────────────────────────
...
def handle(msg: dict, tool_lock=None) -> dict | None:
    """Processa UMA mensagem JSON-RPC e retorna a resposta (dict) ou None (notificação).

    Batch JSON-RPC (array no corpo) não é suportado — nenhum cliente MCP real usa
    batch; um objeto não-dict recebe erro -32600 documentado, em vez de estourar.

    `tool_lock` (opcional): um lock adquirido SÓ em volta da execução da tool
    (`tools/call`), não do dispatch inteiro. O parse, o roteamento e as respostas
    de `initialize`/`tools/list` seguem concorrentes; só a chamada da tool serializa
    (openpyxl não é thread-safe). Sem lock (default) o comportamento é idêntico ao
    servidor stdio original — por isso os testes unitários não precisam de lock.
    """
    if not isinstance(msg, dict):
        return _error(None, -32600, "Requisição inválida: esperado um objeto JSON-RPC (batch não suportado)")
    ...
    if method == "tools/call":
        ...
        if not entry:
            return _error(rid, -32601, f"tool desconhecida: {name}")
        fn = entry[0]
        try:
            ...
            return _result(rid, {"content": [{"type": "text", "text": payload}], "isError": is_err})
        except Exception as exc:  # noqa: BLE001
            logger.error("tool '%s' falhou:\n%s", name, traceback.format_exc())
            return _result(rid, {"content": [{"type": "text", "text": f"ERRO: {type(exc).__name__}: {exc}"}],
                                 "isError": True})
    if rid is not None:
        return _error(rid, -32601, f"método não suportado: {method}")
    return None
